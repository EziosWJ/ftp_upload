...
- **仪表盘** - 系统概览和快速操作
- **设备基本信息** - 设备编码、名称、规格型号、生产厂家等基本信息管理
- **安标信息** - 安标证书管理，包括证书编号、有效期、状态等
- **测点管理** - 测点管理子菜单，包含：
  - 测点管理 — Modbus TCP 或 S7 设备配置
  - 测点基础信息 — 测点编码、类型、量程、报警限等
  - 测点实时信息 — 实时测点数值和状态监控
- **采集计划** - 管理定时采集任务
- **FTP配置** - 配置 FTP 服务器和上传策略
- **系统日志** - 查看系统运行日志
